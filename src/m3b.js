...
................`,
  ...
  APP.mod({
    ...
    mount(root, m) {
      APP.scaffold(root, m, {
        lead: '세선화는 두께 2~3인 에지를 두께 1로 깎되 <b>8-연결성을 끊지 않아야</b> 합니다. SPTA는 에지 화소 p의 여덟 이웃 n0~n7을 보고, p가 <b>경계(껍질)</b> 화소이면서 지워도 <b>끝점이 짧아지거나 연결이 끊기지 않을 때만</b> 지웁니다. 한 패스는 “껍질을 한 번 벗기는” 연산이고, 판단은 항상 <b>원래 e</b>를 보고 결과는 e<sub>out</sub>에 씁니다(빨간 칸 = 이번 패스에서 지워질 화소). 격자에서 화소를 클릭하면 그 화소의 판단 단계로 바로 이동합니다.',
      ...
      const gv = UI.GridView({
        ...
        cell: (y, x) => {
          ...
          if (frame && frame.del && frame.del[y][x]) s.cls = 'del';
          if (frame && frame.cur) {
      ...
      function rebuild(at = 0) {
        ...
        fr.push({ line: 1, del: CV.clone(del), note: `패스 ${pass}: e를 e_out에 복사합니다. 이제 테두리를 뺀 모든 에지 화소를 차례로 검사합니다.`, vars: { 패스: pass } });
        let cnt = 0;
        for (let j = 1; j < M - 1; j++) for (let i = 1; i < N - 1; i++) {
          if (e[j][i] !== 1) continue;
          const n = OFF.map(([a, b]) => e[j + a][i + b]);
          const G = evalGroups(n), win = G.find(g => g.val);
          if (win) { del[j][i] = 1; cnt++; }
          fr.push({ cur: [j, i], n, G, del: CV.clone(del), line: win ? [4, win.line, 9] : [4, 5, 6, 7, 8], vars: { j, i, ...Object.fromEntries(n.map((v, k) => ['n' + k, v])), s0: +G[0].val, s4: +G[1].val, s2: +G[2].val, s6: +G[3].val, '삭제': win ? '예' : '아니오' },
            note: win ? `p=(${j},${i}): <b>${win.key}가 참</b> (${NAMES[win.b]}쪽 경계이고, 지워도 끝점·연결이 유지됨) → <b>e_out(${j},${i}) = 0</b>` : `p=(${j},${i}): 네 식이 모두 거짓 → <b>유지</b>. ` + (G.every(g => !g.lits[0].val) ? '네 방향 모두 에지로 둘러싸인 안쪽 화소입니다.' : '경계이긴 하지만 지우면 끝점이 짧아지거나 연결이 끊깁니다.') });
        }
        fr.push({ line: [], del: CV.clone(del), note: `패스 ${pass} 끝: <b>${cnt}개</b> 화소를 지웁니다. ${cnt ? '“다음 패스”로 e_out을 새 e로 삼아 한 겹 더 벗기세요.' : '더 지울 화소가 없으므로 세선화가 끝났습니다.'}`, vars: { 패스: pass, '지운 화소': cnt } });
      ...
      root.append(h('div', { class: 'lab wide-code' },
        h('div', { class: 'stack' },
          h('div', { class: 'card' },
            h('div', { class: 'controls' }, modeSeg, h('div', { class: 'presets' }, Object.keys(SPTA_PRESETS).map(k => h('button', { class: 'btn', onclick: () => { base = fromArt(SPTA_PRESETS[k]); e = base.map(r => r.slice()); M = e.length; N = e[0].length; gv.resize(M, N); pass = 1; hist = []; rebuild(); } }, k)))),
            h('div', { class: 'controls', style: { marginTop: '8px' } }, passInfo,
  ...
  APP.mod({
    ...
    mount(root, m) {
      ...
      function rebuild() {
        ...
        snap([], `큐가 비어 끝났습니다. 에지 토막 ${segs.length}개.` + (un.length ? ` 방문하지 못한 화소 ${un.length}개 — 끝점·분기점이 없는 닫힌 고리(교재 표의 5번 토막)는 큐에 들어가지 않으므로 알고리즘 3-6만으로는 찾지 못합니다.` : ''));
